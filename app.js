...
async function seedIfEmpty(){
  // Only seed the legacy Amsterdam trip — new trips start completely empty
  if(TRIP_ID) return;
...
window.toggleChk=async id=>{await setDoc(dref('checks',id),{done:!CHECKS[id]});};

// ── NOTES ─────────────────────────────────────────────────
window.addNote=async()=>{const inp=document.getElementById('n-inp');const txt=inp.value.trim();if(!txt)return;await addDoc(col('notes'),{person:AP.i,personAv:AP.a,personName:AP.n,text:txt,date:new Date().toLocaleString('es-AR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}),ts:Date.now()});inp.value='';};
...
function buildDetails(ev){
  ...
  if(tipo==='vuelo'){
    if(x.origen||x.destino) d+=`<div class="edetail full"><label>Ruta</label><p>${x.origen||'—'} → ${x.destino||'—'}</p></div>`;
    if(x.salida||x.llegada) d+=`<div class="edetail"><label>Salida → Llegada</label><p>${x.salida||'—'} → ${x.llegada||'—'}${x.duracion?' ('+x.duracion+')':''}</p></div>`;
    if(x.escala) d+=`<div class="edetail"><label>Escala</label><p>${x.escala}</p></div>`;
    if(x.pnr) d+=`<div class="edetail"><label>Código PNR</label><p style="font-size:15px;letter-spacing:.05em">${x.pnr}</p></div>`;
    if(x.clase) d+=`<div class="edetail"><label>Clase</label><p>${x.clase}</p></div>`;
    if(x.terminal) d+=`<div class="edetail"><label>Terminal</label><p>${x.terminal}</p></div>`;
    if(x.equipaje) d+=`<div class="edetail full"><label>Equipaje</label><p>${x.equipaje}</p></div>`;
    if(x.asientos) d+=`<div class="edetail full"><label>Asientos</label><p>${x.asientos}</p></div>`;
    if(x.notas) d+=`<div class="edetail full"><label>Notas</label><p>${x.notas}</p></div>`;
  }else if(tipo==='hotel'){
    if(x.dir) d+=`<div class="edetail full"><label>Dirección</label><p>${x.dir}${x.maps&&x.maps.startsWith('http')?` <a href="${x.maps}" target="_blank">📍 Maps</a>`:''}</p></div>`;
    if(x.cinFecha||x.cinHora) d+=`<div class="edetail"><label>Check-in</label><p>${x.cinFecha?fmtDateDisplay(x.cinFecha):''} ${x.cinHora||''}</p></div>`;
    if(x.coutFecha||x.coutHora) d+=`<div class="edetail"><label>Check-out</label><p>${x.coutFecha?fmtDateDisplay(x.coutFecha):''} ${x.coutHora||''}</p></div>`;
    if(x.conf) d+=`<div class="edetail"><label>N° confirmación</label><p>${x.conf}</p></div>`;
    if(x.tel) d+=`<div class="edetail"><label>Teléfono</label><p>${x.tel}</p></div>`;
    if(x.notas) d+=`<div class="edetail full"><label>Notas</label><p>${x.notas}</p></div>`;
  }else if(tipo==='actividad'){
    if(x.lugar) d+=`<div class="edetail full"><label>Lugar</label><p>${x.lugar}${x.maps&&x.maps.startsWith('http')?` <a href="${x.maps}" target="_blank">📍</a>`:''}</p></div>`;
    if(x.horafin) d+=`<div class="edetail"><label>Hasta</label><p>${x.horafin}</p></div>`;
    if(x.precio) d+=`<div class="edetail"><label>Precio</label><p>${x.precio}</p></div>`;
    if(x.conf) d+=`<div class="edetail"><label>Confirmación</label><p>${x.conf}</p></div>`;
    if(x.proveedor) d+=`<div class="edetail"><label>Proveedor</label><p>${x.proveedor}</p></div>`;
    if(x.link) d+=`<div class="edetail full"><label>Link</label><p><a href="${x.link}" target="_blank">${x.link}</a></p></div>`;
    if(x.notas) d+=`<div class="edetail full"><label>Notas / tips</label><p>${x.notas}</p></div>`;
  }else if(tipo==='restaurante'){
    if(x.dir||x.nombre) d+=`<div class="edetail full"><label>Dirección</label><p>${x.dir||x.nombre}${x.maps&&x.maps.startsWith('http')?` <a href="${x.maps}" target="_blank">📍</a>`:''}</p></div>`;
    if(x.cocina) d+=`<div class="edetail"><label>Cocina</label><p>${x.cocina}</p></div>`;
    if(x.precio) d+=`<div class="edetail"><label>Precio est.</label><p>${x.precio}</p></div>`;
    if(x.conf) d+=`<div class="edetail"><label>N° reserva</label><p>${x.conf}</p></div>`;
    if(x.reserva) d+=`<div class="edetail"><label>Estado</label><p>${x.reserva}</p></div>`;
    if(x.link) d+=`<div class="edetail full"><label>Link</label><p><a href="${x.link}" target="_blank">${x.link}</a></p></div>`;
    if(x.notas) d+=`<div class="edetail full"><label>Notas</label><p>${x.notas}</p></div>`;
  }else if(tipo==='traslado'){
    if(x.origen||x.destino) d+=`<div class="edetail full"><label>Trayecto</label><p>${x.origen||'—'} → ${x.destino||'—'}</p></div>`;
    if(x.precio) d+=`<div class="edetail"><label>Precio est.</label><p>${x.precio}</p></div>`;
    if(x.conf) d+=`<div class="edetail"><label>N° / línea</label><p>${x.conf}</p></div>`;
    if(x.notas) d+=`<div class="edetail full"><label>Notas</label><p>${x.notas}</p></div>`;
  }else{
    const det=ev.det||x.notas||'';
    if(det) d+=`<div class="edetail full"><label>Detalle</label><p>${det}</p></div>`;
    if(x.lugar) d+=`<div class="edetail full"><label>Lugar</label><p>${x.lugar}</p></div>`;
  }
  return d;
}
...
function renderEventCard(ev,dayId,j){
  ...
  let keyInfo='';
  if(tipo==='vuelo'&&x.origen&&x.destino) keyInfo=`${x.origen.split('—')[0].trim()} → ${x.destino.split('—')[0].trim()}${x.pnr?' · '+x.pnr:''}`;
  else if(tipo==='hotel'&&(x.nombre||x.ciudad)) keyInfo=`${x.nombre||''}${x.cinHora?' · Check-in '+x.cinHora:''}${x.conf?' · '+x.conf:''}`;
  else if(tipo==='traslado'&&x.medio) keyInfo=`${x.medio}${x.duracion?' · '+x.duracion:''}${x.origen?' · '+x.origen.split('—')[0]:''} → ${(x.destino||'').split('—')[0]}`;
  else if(tipo==='restaurante') keyInfo=`${x.nombre||x.dir||''}${x.cocina?' · '+x.cocina:''}${x.reserva?' · '+x.reserva:''}`;
  else if(tipo==='actividad') keyInfo=`${x.lugar||''}${x.precio?' · '+x.precio:''}`;
  else if(tipo==='otro') keyInfo=x.lugar||x.notas?.slice(0,60)||'';
    ...
    <div class="ecard-hd" onclick="${hasDetails?`toggleEcard('${cid}')`:''}" style="${hasDetails?'cursor:pointer':''}">
      ...
      <div class="ecard-main">
        <div class="ecard-title">
          ...
          <span style="font-size:16px;font-weight:700">${titulo}</span>
  ...
  wrap.innerHTML=getDisplayDays().map(d=>{
    ...
    const evHtml=evs.length===0
      ...
      :evs.map((ev,j)=>{
          ...
          const x=ev.extras||{};
          let sub='';
          if(tipo==='vuelo'&&x.origen&&x.destino)sub=`${x.origen.split('—')[0].trim()} → ${x.destino.split('—')[0].trim()}${x.pnr?' · '+x.pnr:''}`;
          else if(tipo==='hotel')sub=`Check-in${x.cinHora?' '+x.cinHora:''}${x.noch?' · '+x.noch:''}`;
          else if(tipo==='traslado'&&x.medio)sub=`${x.medio}${x.duracion?' · '+x.duracion:''}`;
          else if(tipo==='actividad'&&x.lugar)sub=x.lugar;
          else if(tipo==='restaurante'&&(x.nombre||x.dir))sub=x.nombre||x.dir;
          ...
          return `<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:.5px solid rgba(60,60,67,.1);cursor:pointer" onclick="openEditEv('${d._id}',${ri})">
            ...
            <div style="flex:1;min-width:0">
              <div style="font-size:14px;font-weight:500;color:#000;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${tit}</div>
              ...
              ${ev._lastEdit?`<div style="font-size:10px;color:rgba(60,60,67,.35);margin-top:2px;font-weight:500">Editado por ${ev._lastEdit.user} · ${timeAgo(ev._lastEdit.ts)}</div>`:''}
            </div>
    ...
    return `<div style="padding:0 0 2px">
      ...
      <div style="background:#fff;border-radius:12px;overflow:hidden;margin:0 0 8px">
        <div style="padding:12px 14px 10px;border-bottom:.5px solid rgba(60,60,67,.08)">
          <div style="font-size:15px;font-weight:600;color:#000;margin-bottom:2px">${h.name}</div>
          <div style="font-size:12px;color:rgba(60,60,67,.5)">${h.city||''}</div>
        </div>
        <div>
    ...
    <div class="cgroup">
      ...
      ${items.map(item=>`
        <div class="citem${CHECKS[item._id||item.id]?' done':''}">
          ...
          <label onclick="toggleChk('${item._id||item.id}')">${item.text}</label>
...
window.renderPhotos=()=>{
  ...
  grid.innerHTML=PHOTOS.map(p=>`
    <div class="photo-thumb" onclick="openViewer('${p.data}','${(p.caption?p.caption+' — ':'')+p.personName+' · '+p.date}')">
      ...
      <div class="photo-owner">${p.personName||FN[p.person]||p.person}</div>
  ...
  document.getElementById('profiles-wrap').innerHTML=tvlList.map(t=>{
    ...
    <div class="profile-card">
      ...
      <div class="profile-fields">
        <div class="pf"><div class="pf-lbl">🍽 Comida</div><div class="pf-val${!p.comida?' pf-empty':''}">${p.comida||'Sin completar'}</div></div>
        <div class="pf"><div class="pf-lbl">🛍 Shopping</div><div class="pf-val${!p.shopping?' pf-empty':''}">${p.shopping||'Sin completar'}</div></div>
        <div class="pf"><div class="pf-lbl">🎯 Actividades</div><div class="pf-val${!p.actividades?' pf-empty':''}">${p.actividades||'Sin completar'}</div></div>
        <div class="pf"><div class="pf-lbl">😴 Ritmo</div><div class="pf-val${!p.ritmo?' pf-empty':''}">${p.ritmo||'Sin completar'}</div></div>
        ${p.nota?`<div class="pf full"><div class="pf-lbl">💬 Nota para el grupo</div><div class="pf-val">${p.nota}</div></div>`:''}
      </div>
